<template>
  <el-row  type="flex" class="test imageCon">
    <el-col :span="7" :offset="8"></el-col>
    <el-col :span="6" :offset="2" id="boxCon">
      <el-row class="box-color-grey transparent" style="height:480px;padding: 20px;margin-top:30px">
        <el-row type="flex" :justify="'center'" style="width:100%;">
          <img src="../assets/logo.png" alt="example pic" height="60">
        </el-row>
        ...
        >
          <el-button
            ...
            size="medium"
            style="width:60%"
            @click="sendCodeButtonClick"
          >发送验证码</el-button>
        </el-row>
        <el-row type="flex" :justify="'center'" v-if="emailCodeSent">
          <el-input placeholder="验证码" v-model="userValidCode" clearable></el-input>
        </el-row>
        ...
        <el-row
          type="flex"
          :justify="'center'"
          :class="{manuallyDown:(!emailValid)&&(!emailCodeSent),manuallyUp:emailValid&&(!emailCodeSent),manuallyMiddle:emailCodeSent}"
        >
          <p style="font-size:14px; margin:18px 20px 20px 20px;">注册即表示你同意接受我们的条款、数据使用政策和 Cookie 政策 。</p>
        </el-row>
      </el-row>
      <el-row
        type="flex"
        :align="'middle'"
        :justify="'center'"
        class="box-color-grey transparent"
...
.imageCon {
  background-image: url("../assets/images/back-1.jpg");
...
export default {
  ...
  created() {
    this.axios
      .get("http://10.0.1.61:51738/api/products/1")
      ///// 箭头函数会改变this的作用域
      .then(response => {
        this.tableData = [];
  ...
  methods: {
    ...
    registerButtonClick() {
      if (this.isEmail(this.email)) {
        ...
        this.axios
          .post("http://192.168.43.249:54468/api/Users/Register", {
            Email: this.email,
  ...
  beforeRouteEnter(to, from, next) {
    ...
    next(vm => {
      vm.axios
        .get("http://192.168.0.37:5000/feed/sss")
        .then(response => {
